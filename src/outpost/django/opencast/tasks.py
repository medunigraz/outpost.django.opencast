...
class GroupMapTask(Task):
    ...
    def process(self, pk):
        ...
        try:
            resp = self.session.get(self.url.path(f"api/v1/groups/{gm.group.lower()}"))
        ...
        try:
            ...
            logger.warn(
                "LDAP query failed when synchronizing Opencast users " f"for {gm}: {e}"
            )
            return
        ...
        for username, email in ldap_group.items():
            ...
            try:
                ...
                logger.warn(
                    f"Failed to fetch details for external user {username}: {e}"
                )
                continue
            ...
            try:
                ...
                logger.warn(
                    f"Failed to fetch details for internal user {username}: {e}"
                )
                update.add(username)
                continue
        ...
        for d in update:
            ...
            if d in members:
                members.remove(d)
        logger.debug(
            f"Updating group {gm} with deprecated and mismatched users removed"
        )
        try:
            self.update_group(
                oc_group.get("name"),
    ...
    def update_group(self, name, roles, members):
        resp = self.session.put(
            ...
            json={"name": name, "roles": roles, "members": members},
        )
        resp.raise_for_status()
    ...
    def get_user_external(self, username):
        ...
        resp = self.session.get(self.url.path(path).as_string())
        resp.raise_for_status()
        return resp.json()
    ...
    def remove_user_external(self, username):
        provider = settings.OPENCAST_LDAP_PROVIDER
        path = f"api/v1/users/providers/{provider}/users/{username}"
        resp = self.session.delete(self.url.path(path).as_string())
        resp.raise_for_status()
